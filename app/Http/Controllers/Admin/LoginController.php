...
class LoginController extends Controller
{
    ...
    protected $redirectTo   = '/admin/company/'; //ログイン後に表示する画面 TODO
    ...
    {
        $this->middleware('auth:administrators')->except(['index', 'login', 'logout', 'reminder', 'pwReset', 'reminderSendMail', 'pwUpdate', 'pwComplete']);
    }
    ...
    {
        return view('admin.login.reminder');
    }
    ...
    public function reminderSendMail(ReminderRequest $request)
    {
        ...
        //ログインIDからユーザーを探す
        $user = User::getAdminUserByLoginId($loginId);
        ...
        //パスワード再設定用画面URL生成
        $setUrl       = route('reset', ['id' => $loginId]) . '/' . $authKey;


        SendMail::sendReminderPasswordMail($mailAddress, $user->administrator->name, $setUrl);

        return redirect('admin/login/reminder')->with('status', 'パスワード再設定用のメールを送信しました');
    }
    ...
    public function pwReset(Request $request)
    {
        ...
        $user = User::getAdminUserByLoginId($login_id);
        ...
        return view('admin.login.pw_reset', $requestDatas);
    }
    ...
    public function pwUpdate(PasswordRequest $request)
    {
        ...
        return redirect('admin/login/pw-complete');
    }
    ...
    {
        return view('admin.login.pw_reset_complete');
    }
}
