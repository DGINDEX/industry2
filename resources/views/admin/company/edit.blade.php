...
@section('content')
    <div class="card">
        ...
        <!-- /.card-header -->
        {{ Form::open(['url' => '/admin/company/update', 'method' => 'post', 'id' => 'form', 'name' => 'form', 'files' => true]) }}
        ...
        <div class="card-body">
            <table class='table table-bordered tbl-detail'>
                ...
                <tr>
                    ...
                    <th>ステータス <span class="badge badge-danger">必須</span></th>
                    <td>
                        {{ Form::select('status', config('const.status_name'), $company->status, ['class' => 'form-control']) }}
                    </td>
                </tr>
                <tr>
                            ...
                            <td>
                                @if (isset($id))
                                    <button type="button" class="btn btn-outline-danger float-center"
                                        onclick="dataDelete(event, '{{ url('/admin') }}/company/image-delete/{{ $id }}', '#form', 'post');">削除</button>
                                @endif
                            </td>
                        </tr>
                    @endfor
        ...
        <div class="card-footer">
            @if ($company->id)
                <button type="button" class="btn btn-danger float-left delete"
                    onclick="dataDelete(event, '{{ url('/admin') }}/company/delete/{{ $company->id }}', '#form', 'post');">削除</button>
            @endif
            <button id="save-button" type="submit" class="btn btn-primary float-right">保存</button>
            <a href="{{ url('/admin') }}/company" class="btn btn-secondary float-right mr-2">戻る</a>
        </div>
        {{ Form::close() }}
        <!-- /.card-footer -->
    </div>
