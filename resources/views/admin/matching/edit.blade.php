...
@section('content')
    <div class="card">
        ...
        <!-- /.card-header -->
        {{ Form::open(['url' => '/admin/matching/update', 'method' => 'post', 'id' => 'form', 'name' => 'form', 'files' => true]) }}
        ...
        <div class="card-body">
            <table class='table table-bordered tbl-detail'>
                <tr>
                    ...
                    <td colspan="3">
                        {{ Form::select('company_id', $companyList, $matching->company_id, ['class' => 'form-control pull-right', 'placeholder' => '選択してください']) }}
                    </td>
                </tr>
                <tr>
                            ...
                            <td>
                                @if (isset($id))
                                    <button type="button" class="btn btn-outline-danger float-center"
                                        onclick="dataDelete(event, '{{ url('/admin') }}/matching/image-delete/{{ $id }}', '#form', 'post');">削除</button>
                                @endif
                            </td>
                        </tr>
                    @endfor
        ...
        <div class="card-footer">
            @if ($matching->id)
                <button type="button" class="btn btn-danger float-left delete"
                    onclick="dataDelete(event, '{{ url('/admin') }}/matching/delete/{{ $matching->id }}', '#form', 'post');">削除</button>
            @endif
            <button id="save-button" type="submit" class="btn btn-primary float-right">保存</button>
            <a href="{{ url('/admin') }}/matching" class="btn btn-secondary float-right mr-2">戻る</a>
        </div>
        {{ Form::close() }}
        <!-- /.card-footer -->
